...
namespace plant_nanny::services::ota
{
    ...
    {
        auto validation = s_ota_state.validate_updating();
        if (validation.failed()) return common::patterns::Result<size_t>::failure(validation.error());
    ...
    common::patterns::Result<void> Manager::finalize_update()
    {
        ...
        if (!EspError::is_ok(err))
        {
            LOG_IF_AVAILABLE(logger_, error, err == ESP_ERR_OTA_VALIDATE_FAILED ? 
                "Image validation failed, image is corrupted" : "Failed to end OTA update");
            s_ota_state.reset();
            return err == ESP_ERR_OTA_VALIDATE_FAILED ?
                common::patterns::Result<void>::failure(common::patterns::Error("OTA image validation failed - image is corrupted")) :
                EspError::to_result(err, "Failed to end OTA update");
        }
    ...
    common::patterns::Result<void> Manager::rollback()
    {
        ...
        if (!EspError::is_ok(err)) return EspError::to_result(err, "Failed to get OTA state");

        if (ota_state == ESP_OTA_IMG_PENDING_VERIFY)
        {
            err = esp_ota_mark_app_invalid_rollback_and_reboot();
            if (!EspError::is_ok(err)) return EspError::to_result(err, "Failed to rollback");
        }
